...
import { User, EchoEntry, CircleRoom, FundraisingProposal } from './types';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const initialEchoes = db.getPulses() as unknown as EchoEntry[];
    setProposals(db.getProposals());
    if (initialEchoes.length === 0) {
      const welcome: EchoEntry = {
        id: 'welcome',
        authorId: 'system',
        authorName: 'Echo Guide',
        title: 'The Sanctuary is Open',
        content: 'Your thoughts are now sovereign. Every Echo you publish here is encrypted. Every Circle you join is anonymous. Welcome home.',
        timestamp: Date.now(),
        stats: { reads: 1, plays: 0, likes: 0 },
        comments: []
      };
      db.savePulse(welcome as any);
      setEchoes([welcome]);
    } else {
  ...
  const handlePublish = (e: EchoEntry) => {
    db.savePulse(e as any);
    setEchoes([e, ...echoes]);
  ...
  return (
    <div className="min-h-screen bg-background text-accent flex flex-col selection:bg-serene selection:text-white">
      <header className="sticky top-0 z-[100] bg-white/80 backdrop-blur-2xl border-b border-border px-8 py-6">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex flex-col cursor-pointer group" onClick={() => setActiveView('echoes')}>
            <h1 className="text-2xl font-black tracking-tighter uppercase font-mono group-hover:text-serene transition-colors">Echoes & Circles</h1>
            <span className="text-[10px] text-dim font-mono tracking-[0.4em] uppercase opacity-50">High Integrity Sanctuary</span>
          </div>
          ...
          <div className="flex items-center gap-10">
             ...
             <div className="flex items-center gap-6">
               <div className="flex flex-col items-end">
                 <span className="text-[9px] font-mono uppercase text-dim tracking-widest">Resonance</span>
                 <span className="text-base font-bold tracking-tighter text-serene">{user.auraScore}Hz</span>
               </div>
               <img 
                 src={user.avatar} 
                 onClick={() => setActiveView('profile')}
                 className={`w-12 h-12 border-2 p-1 transition-all cursor-pointer hover:scale-110 shadow-lg ${activeView === 'profile' ? 'border-serene' : 'border-accent'}`} 
                 alt="Identity Avatar" 
               />
             </div>
          </div>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-8 sm:p-20 mb-48">
